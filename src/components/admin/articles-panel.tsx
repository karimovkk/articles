...
import { Alert, Badge, Button, Field, Input, Modal, Select, statusTone } from "@/components/ui";
...
export function ArticlesPanel({ bookId, articles, onChanged }: { bookId: string; articles: Article[]; onChanged: () => void }) {
  ...
  function move(a: Article, dir: -1 | 1) {
    ...
    if (!other) return;
    void run(a.id, () => Promise.all([adminApi.updateArticle(bookId, a.id, { order_index: other.order_index }), adminApi.updateArticle(bookId, other.id, { order_index: a.order_index })]));
  }
  ...
  async function onFile(file: File) {
    ...
    await run(id, () => adminApi.uploadArticleFile(bookId, id, file, { onProgress: (loaded, total) => setUpload({ id, loaded, total }) }));
    setUpload(null);
  }
    ...
    <div className="space-y-3">
              ...
              <tr key={a.id} data-article={a.id}>
                <td className="py-2 align-top text-muted">
                  <span className="inline-flex items-center gap-0.5">
                    {i + 1}
                    <button className="px-1 text-xs hover:text-text disabled:opacity-30" disabled={i === 0 || !!busyId} onClick={() => move(a, -1)} title={t("admin.articles.up")} aria-label={t("admin.articles.up")}>
                      ↑
                    </button>
                    <button className="px-1 text-xs hover:text-text disabled:opacity-30" disabled={i === sorted.length - 1 || !!busyId} onClick={() => move(a, 1)} title={t("admin.articles.down")} aria-label={t("admin.articles.down")}>
                      ↓
                    </button>
                  </span>
                </td>
                ...
                <td className="py-2 align-top">
                  <Badge tone={statusTone(a.processing_status)}>{a.processing_status}</Badge>
                  {a.processing_error && <p className="mt-1 text-xs text-red-500">{a.processing_error}</p>}
                </td>
                <td className="py-2 align-top text-muted">{a.page_count ?? "—"}</td>
                ...
                <td className="py-2 text-right align-top">
                  <div className="flex flex-wrap justify-end gap-1">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => openToc(a)}>
                    ...
                    <Button
                      ...
                      loading={busyId === a.id}
                      onClick={() => {
                        if (confirm(t("admin.articles.deleteConfirm", { title: a.title }))) void run(a.id, () => adminApi.deleteArticle(bookId, a.id));
                      }}
                    >
                      {t("common.delete")}
                    </Button>
      ...
      <form onSubmit={create} className="flex flex-wrap items-end gap-2">
        <Field label={t("admin.articles.newTitle")}>
          <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder={t("admin.articles.newPlaceholder")} className="min-w-[260px]" />
        </Field>
        <Button type="submit" loading={busyId === "new"} disabled={!newTitle.trim()}>
          {t("admin.articles.add")}
        </Button>
        <p className="w-full text-xs text-muted">{t("admin.articles.hint", { pdf: env.maxPdfMb })}</p>
      </form>

      <Modal open={!!renaming} onClose={() => setRenaming(null)} title={t("admin.articles.rename")}>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!renaming) return;
      ...
      <Modal open={!!tocFor} onClose={() => setTocFor(null)} title={`${t("admin.articles.toc")}: ${tocFor?.title ?? ""}`}>
        <form onSubmit={saveToc} className="space-y-3">
          <p className="text-xs text-muted">{t("admin.articles.tocHint")}</p>
          <div className="max-h-80 space-y-2 overflow-auto">
            {tocRows.map((r, i) => (
              <div key={i} className="flex items-center gap-2">
                <Select value={r.level} onChange={(e) => setTocRows((rows) => rows.map((x, j) => (j === i ? { ...x, level: Number(e.target.value) } : x)))} className="w-20" aria-label="level">
                  {[1, 2, 3].map((l) => (
                ...
                <Input value={r.page} onChange={(e) => setTocRows((rows) => rows.map((x, j) => (j === i ? { ...x, page: e.target.value } : x)))} placeholder={t("common.page")} inputMode="numeric" className="w-20" />
                <button type="button" className="text-muted hover:text-red-500" onClick={() => setTocRows((rows) => rows.filter((_, j) => j !== i))} aria-label={t("common.delete")}>
                  ✕
                </button>
              </div>
            ))}
          </div>
          <Button type="button" size="sm" variant="secondary" onClick={() => setTocRows((rows) => [...rows, { level: 1, title: "", page: "" }])}>
