...
export function AdminBookDetail({ bookId }: { bookId: string }) {
  ...
  const [edits, setEdits] = useState<Partial<{ title: string; author: string; description: string; category_id: string; price: string }>>({});
  const form = {
  ...
  async function uploadCover(file: File) {
    ...
    await run(t("admin.books.coverUploaded"), () => adminApi.uploadCover(bookId, file, { onProgress: (loaded, total) => setUpload({ label, loaded, total }) }));
    setUpload(null);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-6">
          <Card className="p-5">
            <h2 className="mb-1 text-base font-semibold text-text">{t("admin.books.cover")}</h2>
            <p className="mb-3 text-xs text-muted">{t("admin.books.coverLimit", { cover: env.maxCoverMb })}</p>
          ...
          <Card className="p-5">
            ...
            <Button
              ...
              title={nextStatus === "ACTIVE" && readyArticles === 0 ? t("admin.books.uploadFirst") : undefined}
              onClick={() => run(t("admin.books.statusSet", { s: nextStatus }), () => adminApi.updateBook(bookId, { status: nextStatus }))}
            >
              {nextStatus === "ACTIVE" ? t("common.activate") : t("common.deactivate")}
            </Button>
          </Card>
        </div>
      </div>
      ...
      <Card className="p-5">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-base font-semibold text-text">{t("admin.books.grantedTo", { n: access.filter((a) => a.status === "ACTIVE").length })}</h2>
